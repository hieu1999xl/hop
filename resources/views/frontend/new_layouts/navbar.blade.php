<header class="header-top">
    @php
        ...
        $items_services = App\Models\Menu::where('parent_id', '=', '65')
        -> whereNotIn('id' , [74 , 75])
        ->where('status', '=', '1')->where('order', '>', '0')->orderBy('order')->get();
    @endphp
    <div class="container">
        ...
        <div class="row ">
            <div class="col-md-2 col-4 nav_header">
                <a href="{{ route('frontend.index') }}">
                    <img class="logo_header" src="{{ asset('frontend/assets/images/HOP/logo06.png')}}" alt="logo"/>
                </a>
            </div>
                ...
                <ul class="nav-lists nav_header">
                    @foreach($items_menu as $item)
                        ...
                        <li data-id="{{$item->slug}}">
                            <a href="{{ url($url)  }}"
                               class="font_weight_500 header_link">{{trans('frontend.'. $item->name)}}</a>
                            <div></div>
                        </li>
                    @endforeach
                </ul>
                <div class="nav-lists-mobile">
                    <div class="link pl-3 py-4 font-weight-bold">MENU</div>
                    <ul id="accordion" class="accordion menu-mobile mt-0">
                        @foreach($items_menu as $menu)
                            ...
                            <li class="link_mobile">
                                <div class="link">
                                    <a href="{{count($items_child_lv1) > 0 && $menu->id != 70 ? '#' : '/'.$menu->slug}}"
                                       class="">{{trans('frontend.'.$menu->name)}}</a>
                                ...
                                @if (count($items_child_lv1) > 0)
                                    <ul class="submenu">
                                        @php
                                            $items_child= App\Models\Menu::where('parent_id', '=', 73)->where('status', '=', '1')->orderBy('order')->get();
                                        @endphp
                                        @foreach($items_child as $item_child)
                                            ...
                                            <li>
                                                <a href="{{$url}}" target="{{$target}}"
                                                   rel="{{ $rel }}">{{trans('services.' . $item_child->name)}}</a>
                                            </li>
                                        @endforeach
                                    </ul>
                                @endif
            ...
            <div class="col-10 box-search-pc">
                ...
                <form id="form_search" class="info" action="{{ route('frontend.search_result') }}" method="GET">
                    <input id="searchKey" value="" name="querySearchKey" type="search"
                           placeholder="{{trans('frontend.searchheader')}}">
                    ...
                    @foreach($items_services as $key => $item)
                        ...
                        @foreach($items_child as $item_child)
                            ...
                            <li class="list_menu_child_not_border">
                                <a href="{{ $url }}" class="sub_header_child_link" target="{{$target}}"
                                   rel="{{ $rel }}">
                                    {{trans('services.' . $item_child->name)}}
                                </a>
                            </li>
                        @endforeach
            ...
            <div class="offset-2 col-10">
                <ul>
                    @foreach($items_industries as $item)
                        <li class="list_menu_child_not_border">
                            <a href="{{ route('frontend.industry_detail', [substr($item->slug, 11)]) }}"
                               class="sub_header_child_link">{{trans('frontend.' . $item->name)}}</a>
                        </li>
                    @endforeach
                </ul>
            </div>
